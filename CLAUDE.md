...
- Platform detection lives in ONE place: `cx/platform.mk`. Every makefile `include`s it (relative path: `../platform.mk` for cx modules, `../../cx/platform.mk` for cx_tests and cx_apps) instead of carrying its own `uname` block. It defines `UNAME_S`, `UNAME_R`, `ARCH`, `PLATFORM_OS`, `PLATFORM` (= `$(PLATFORM_OS)_$(ARCH)`, the per-platform output dir token), `CPPFLAGS`, and `PLATFORM_LIBS`. When adding a makefile, include it and use `$(PLATFORM)` for object/lib dirs — do NOT paste a detection block back in. Module-specific build gates (e.g. regex's `BUILD_REGEX`, ss's `DEVELOPER`) go in the module makefile AFTER the include, not in `platform.mk`.
- `PLATFORM_OS` encodes the OS release for the sun family (`sunos4` / `solaris6` / `solaris10`) so SunOS-4.x and Solaris binaries don't collide in one lib dir. `linux` and `darwin` keep `PLATFORM_OS == uname_s`, so `lib/linux_x86_64` and `lib/darwin_arm64` are unchanged. The compile macro in `CPPFLAGS` (`-D _SUNOS_` etc., what the `#ifdef`s see) is separate from the dir token and unchanged.
